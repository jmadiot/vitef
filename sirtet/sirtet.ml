...
(** Shape fitting in grid **)

type zone = bool array array;;

let fit : zone -> shape -> bool = fun zone ->
	let w,h = dim zone in
	List.fold_left (fun b {x=x;y=y} -> b && x>=0 && y>=0 && x<w && y<h && zone.(y).(x)) true
;;
